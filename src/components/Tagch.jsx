...
import View from "../assets/images/tag_view.svg"; // 이 부분은 제거할 수 있습니다.
...
const Tag = ({ onTagClick }) => {
    ...
    const sortTags = [
        { id: 1, name: "투표수", icon: Like } // "조회수" 항목 제거
    ];
    ...
    const handleClick = (tag) => {
        ...
        if (tag.id === 0) {
            ...
            newSelected.sort = "video.likeCount"; // "투표" 관련 정렬 기준 설정
        ...
        <div className="flex tag-box align-center">
            ...
            {sortTags.map(tag => (
                <Button
                    key={tag.id}
                    size="tag"
                    className={selected.sort === "video.likeCount" ? "selected" : ""}
